...
describe('PATCH /api/admin/periods/:periodId/assignQuantifiers', () => {
  ...
  it('200 response with json body containing updated period', async function () {
    ...
    await seedPraise({ receiver: receiver1._id, createdAt: faker.date.past() });
    await seedPraise({ receiver: receiver1._id, createdAt: faker.date.past() });
    await seedPraise({ receiver: receiver1._id, createdAt: faker.date.past() });
    await seedPraise({ receiver: receiver1._id, createdAt: faker.date.past() });
    await seedPraise({ receiver: receiver1._id, createdAt: faker.date.past() });

    await seedPraise({ receiver: receiver2._id, createdAt: faker.date.past() });
    await seedPraise({ receiver: receiver2._id, createdAt: faker.date.past() });
    await seedPraise({ receiver: receiver2._id, createdAt: faker.date.past() });
    await seedPraise({ receiver: receiver2._id, createdAt: faker.date.past() });

    await seedPraise({ receiver: receiver3._id, createdAt: faker.date.past() });
    await seedPraise({ receiver: receiver3._id, createdAt: faker.date.past() });
    await seedPraise({ receiver: receiver3._id, createdAt: faker.date.past() });
