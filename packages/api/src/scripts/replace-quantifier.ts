 ...
 */
const replaceActiveQuantifier = async (periodId: string, currentQuantifierId: string, newQuantifierId: string) => {
    const period: PeriodDocument | null = await PeriodModel.findOne({ _id: periodId });
...
const run = async (): Promise<void> => {
    const argv: Arguments = yargs.command("$0", 'Replace a specified quantifier who is currently assigned')
        .option('periodId', {
            type: 'string',
