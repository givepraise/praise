...
const praiseQuery = async (
  ...
  const result = await conn.query(`SELECT         
      ...
      CAST(SUM(score) AS INTEGER) as score,
...
const PraiseScoreOverTime = ({
  ...
  React.useEffect(() => {
    ...
    const run = async (): Promise<void> => {
      ...
      const graphData: GraphData = {
        score: await scoreQuery(conn, date2, date3),
        scorePrevious: await scoreQuery(conn, date1, date2),
  ...
  return (
    <Graph.Frame>
      ...
      <Graph.HeaderNumber value={data.score} />
      <Graph.LargeValueChangePill
        value1={data.score}
        value2={data.scorePrevious}
      />
      <Graph.SubHeader>Praise score over time</Graph.SubHeader>
        ...
        <Line
          data={{
            ...
            datasets: [
              ...
              {
                data: data.dataPrevious.map((d) => d.score),
                borderWidth: 1,
                label: 'Previous',
                borderDash: [5, 5],
                pointRadius: 0,
...
export default PraiseScoreOverTime;
