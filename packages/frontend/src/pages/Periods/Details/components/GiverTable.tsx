...
const GiverTable = (): JSX.Element | null => {
  const { periodId } = useParams<PeriodPageParams>();
  const isAdmin = useRecoilValue(HasRole(ROLE_ADMIN));
  useLoadSinglePeriodDetails(periodId);
  const period = useRecoilValue(SinglePeriod(periodId));

  const GiverTableInner = (): JSX.Element => {
    ...
    const data = period?.givers
      ? sortBy(period.givers, [
          // First, sort by reciever score
          (giver): number => {
            if (!giver?.scoreRealized) return 0;
            return giver.scoreRealized;
          },

          // Then by giver _id
          (giver): string => giver._id.toString(),
        ])
      : [];
    ...
    const handleClick = (data: any) => (): void => {
      history.push(`/periods/${periodId}/giver/${data._id}`);
    };
  ...
  if (
    !period.givers ||
    (Array.isArray(period.givers) && period.givers.length) === 0
  )
    return (
      <div className="flex items-center justify-center w-full h-full">
        No givers found in this period.
      </div>
    );

  if (period.givers) return <GiverTableInner />;
