...
export const useAllPeriodsQuery = (): AxiosResponse<unknown> => {
  ...
  const allPeriodsQueryResponse = useAuthApiQuery(
    AllPeriodsQuery(location.key)
  );
...
/**
 * Stores all praise assigned to the currently active quantifier for a period
 */
export const PeriodQuantifierPraise = atomFamily<
  PraiseDto[] | undefined,
  string
>({
  key: 'PeriodQuantifierPraise',
  default: undefined,
});
...
export const usePeriodQuantifierPraiseQuery = (
  ...
  const [periodQuantifierPraise, setPeriodQuantifierPraise] = useRecoilState(
    PeriodQuantifierPraise(periodId)
  );
  React.useEffect(() => {
    if (!periodQuantifierPraise && isResponseOk(response)) {
      setPeriodQuantifierPraise(response.data);
    }
  }, [response]);
  return periodQuantifierPraise;
};
...
//     ) {
//       const praiseList = periodPraiseQueryResponse.data as PraiseDto[];
//       if (Array.isArray(praiseList) && praiseList.length > 0) {
...
//             (item) => item.username === praiseItem.receiver.name
//           );

//           const rd: ReceiverData = {
//             receiverId: praiseItem.receiver._id!,
//             username: praiseItem.receiver.name,
...
// export const ActiveUserQuantifierDataForPeriod = selectorFamily({
//   key: 'ActiveUserQuantifierDataForPeriod',
//   get:
//     (periodId: string) =>
//     ({ get }): PeriodDetailsQuantifierDto | undefined => {
...
export const PeriodActiveQuantifierReceivers = selectorFamily({
  ...
  get:
    (periodId: string) =>
    ({ get }) => {
      const praiseList = get(PeriodQuantifierPraise(periodId));
      const userId = get(ActiveUserId);
      if (praiseList) {
...
type PeriodActiveQuantifierReceiverParams = {
  periodId: string;
  receiverId: string;
};
export const PeriodActiveQuantifierReceiver = selectorFamily({
  ...
  get:
    (params: PeriodActiveQuantifierReceiverParams) =>
    ({ get }) => {
      const { periodId, receiverId } = params;
