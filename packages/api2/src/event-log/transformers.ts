...
import { EventLogTypeDocument } from './entities/event-log-type.entity';
import { EventLogDocument } from './entities/event-log.entity';
...
export const eventLogListTransformer = async (
  ...
  const eventLogDtos = await Promise.all(
    eventLogs.map((eventLog) =>
      eventLogTransformer(eventLog, currentUserRoles),
    ),
  );
